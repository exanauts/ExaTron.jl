                       ...
                       double *pd, double *qd, double *u, double *v, double *l, double *rho, double *YshR, double *YshI)
{
    ...
    if (I < nbuses) {
        ...
        for (int k = fr_start[I]; k < fr_start[I+1]; k++) {
            common += l[wi_i_ij_start + fr_idx[k]] + rho[wi_i_ij_start + fr_idx[k]]*u[wi_i_ij_start + fr_idx[k]];
        ...
        for (int k = to_start[I]; k < to_start[I+1]; k++) {
            common += l[wi_j_ji_start + to_idx[k]] + rho[wi_j_ji_start + to_idx[k]]*u[wi_j_ji_start + to_idx[k]];
        ...
        //mu = A \ [rhs1 ; rhs2]
        double wi = common + ( (YshR[I]*mu1 - YshI[I]*mu2) / rhosum_wi_ij_ji );
