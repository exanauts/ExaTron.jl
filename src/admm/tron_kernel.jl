...
@inline function tron_generator_kernel(
    ...
    while search
        ...
        if task == 0 || task == 1
            f = eval_f_generator_kernel_two_level(scale, x, param, c2, c1, baseMVA)
            nfev += 1
            if nfev >= max_feval
                search = false
            end
        ...
        if task == 0 || task == 2
            eval_g_generator_kernel_two_level(scale, x, g, param, c2, c1, baseMVA)
            eval_h_generator_kernel_two_level(scale, x, A, param, c2, c1, baseMVA)
            ngev += 1
            nhev += 1
            minor_iter += 1
        end
