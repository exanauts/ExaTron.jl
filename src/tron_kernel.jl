...
"""
@inline function tron_kernel(n::Int, shift::Int, max_feval::Int, max_minor::Int, gtol::Float64, use_polar::Bool,
    ...
    while search
        ...
        if task == 0 || task == 1
            if use_polar
                f = eval_f_polar_kernel(n, shift, x, param, YffR, YffI, YftR, YftI, YttR, YttI, YtfR, YtfI)
            else
        ...
        if task == 0 || task == 2
            if use_polar
                eval_grad_f_polar_kernel(n, shift, x, g, param, YffR, YffI, YftR, YftI, YttR, YttI, YtfR, YtfI)
                eval_h_polar_kernel(n, shift, x, A, param, YffR, YffI, YftR, YftI, YttR, YttI, YtfR, YtfI)
            else
