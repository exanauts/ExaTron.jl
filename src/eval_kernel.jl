function eval_f_kernel(n::Int, x::CuDeviceArray{Float64},
    ...
    #f += sum(param[i,I]*x[i,I] for i=1:6)
    #f += 0.5*sum(param[6+i,I]*(x[i,I] - param[12+i,I])^2 for i=1:6)
...
function eval_grad_f_kernel(n::Int, x::CuDeviceArray{Float64}, g::CuDeviceArray{Float64},
    ...
    #=
    @inbounds for i=1:6
        g[i,I] += param[i,I] + param[6+i,I]*(x[i,I] - param[12+i,I])
    end
    =#
    ...
    g5 += param[19,I]*(-YffR[I]) + param[20,I]*(YffI[I]) + param[23,I]*(-x[6]) +
                raug*(-YffR[I])*c1 + raug*(YffI[I])*c2 + raug*(-x[6])*c5
    g6 += param[21,I]*(-YttR[I]) + param[22,I]*(YttI[I]) + param[23,I]*(-x[5]) +
                raug*(-YttR[I])*c3 + raug*(YttI[I])*c4 + raug*(-x[5])*c5
    g7 = param[19,I]*(-YftR[I]) + param[20,I]*(YftI[I]) + param[21,I]*(-YtfR[I]) +
                param[22,I]*(YtfI[I]) + param[23,I]*(2*x[7]) +
                raug*(-YftR[I])*c1 + raug*(YftI[I])*c2 + raug*(-YtfR[I])*c3 +
                raug*(YtfI[I])*c4 + raug*(2*x[7])*c5
    g8 = param[19,I]*(-YftI[I]) + param[20,I]*(-YftR[I]) + param[21,I]*(YtfI[I]) +
    ...
    #=
    g[1,I] += param[19,I] + raug*c1
    g[2,I] += param[20,I] + raug*c2
    g[3,I] += param[21,I] + raug*c3
    g[4,I] += param[22,I] + raug*c4
    g[5,I] += param[19,I]*(-YffR[I]) + param[20,I]*(YffI[I]) + param[23,I]*(-x[6,I]) +
                raug*(-YffR[I])*c1 + raug*(YffI[I])*c2 + raug*(-x[6,I])*c5
    g[6,I] += param[21,I]*(-YttR[I]) + param[22,I]*(YttI[I]) + param[23,I]*(-x[5,I]) +
                raug*(-YttR[I])*c3 + raug*(YttI[I])*c4 + raug*(-x[5,I])*c5
    g[7,I] += param[19,I]*(-YftR[I]) + param[20,I]*(YftI[I]) + param[21,I]*(-YtfR[I]) +
                param[22,I]*(YtfI[I]) + param[23,I]*(2*x[7,I]) +
                raug*(-YftR[I])*c1 + raug*(YftI[I])*c2 + raug*(-YtfR[I])*c3 +
                raug*(YtfI[I])*c4 + raug*(2*x[7,I])*c5
    g[8,I] += param[19,I]*(-YftI[I]) + param[20,I]*(-YftR[I]) + param[21,I]*(YtfI[I]) +
                param[22,I]*(YtfR[I]) + param[23,I]*(2*x[8,I]) +
                raug*(-YftI[I])*c1 + raug*(-YftR[I])*c2 + raug*(YtfI[I])*c3 +
                raug*(YtfR[I])*c4 + raug*(2*x[8,I])*c5
    =#
...
function eval_h_kernel(n::Int, x::CuDeviceArray{Float64}, A::CuDeviceArray{Float64},
    ...
    # 5th column
    A[5,5] = param[11,I] + raug*(YffR[I]^2) + raug*(YffI[I]^2) + raug*(x[6]^2)
    A[6,5] = -(alrect + raug*c5) + raug*(x[5]*x[6])
    A[7,5] = raug*(YffR[I]*YftR[I]) + raug*(YffI[I]*YftI[I]) + raug*((-x[6])*(2*x[7]))
    A[8,5] = raug*(YffR[I]*YftI[I]) + raug*(YffI[I]*(-YftR[I])) + raug*((-x[6])*(2*x[8]))

    # 6th column
    A[6,6] = param[12,I] + raug*(YttR[I]^2) + raug*(YttI[I]^2) + raug*(x[5]^2)
    A[7,6] = raug*(YttR[I]*YtfR[I]) + raug*(YttI[I]*YtfI[I]) + raug*((-x[5])*(2*x[7]))
    A[8,6] = raug*((-YttR[I])*YtfI[I]) + raug*(YttI[I]*YtfR[I]) + raug*((-x[5])*(2*x[8]))

    # 7th column
    A[7,7] = (alrect + raug*c5)*2 + raug*(YftR[I]^2) + raug*(YftI[I]^2) +
        raug*(YtfR[I]^2) + raug*(YtfI[I]^2) + raug*((2*x[7])*(2*x[7]))
    A[8,7] = raug*(YftR[I]*YftI[I]) + raug*(YftI[I]*(-YftR[I])) + raug*((-YtfR[I])*YtfI[I]) +
        raug*(YtfI[I]*YtfR[I]) + raug*((2*x[7])*(2*x[8]))

    # 8th column
    A[8,8] = (alrect + raug*c5)*2 + raug*(YftI[I]^2) + raug*(YftR[I]^2) +
...
#=
function eval_f_kernel_cpu(nline, x, ret,
    param, YffR, YffI, YftR, YftI, YttR, YttI, YtfR, YtfI)
    Threads.@threads for I=1:nline
        @inbounds begin
            f = 0
            f += sum(param[i,I]*x[i,I] for i=1:6)
            f += 0.5*sum(param[6+i,I]*(x[i,I] - param[12+i,I])^2 for i=1:6)

            c1 = (x[1,I] - (YffR[I]*x[5,I] + YftR[I]*x[7,I] + YftI[I]*x[8,I]))
            c2 = (x[2,I] - (-YffI[I]*x[5,I] - YftI[I]*x[7,I] + YftR[I]*x[8,I]))
            c3 = (x[3,I] - (YttR[I]*x[6,I] + YtfR[I]*x[7,I] - YtfI[I]*x[8,I]))
            c4 = (x[4,I] - (-YttI[I]*x[6,I] - YtfI[I]*x[7,I] - YtfR[I]*x[8,I]))
            c5 = (x[7,I]^2 + x[8,I]^2 - x[5,I]*x[6,I])
...
function eval_grad_f_kernel_cpu(nline, x, g,
    param, YffR, YffI, YftR, YftI, YttR, YttI, YtfR, YtfI)
    g .= 0.0
    Threads.@threads for I=1:nline
        @inbounds begin
...
function eval_h_kernel_cpu(nline, mode, scale, rows, cols, lambda,
    x, values, param, YffR, YffI, YftR, YftI, YttR, YttI, YtfR, YtfI)
    Threads.@threads for I=1:nline
        @inbounds begin
            # Sparsity pattern of lower-triangular of Hessian.
            #     1   2   3   4   5   6   7   8
            ...
            if mode == :Structure
                ...
                # 1st column
                # (1,1)
                values[nz,I] = param[7,I] + raug
                nz += 1
                # (5,1)
                values[nz,I] = raug*(-YffR[I])
                nz += 1
                # (7,1)
                values[nz,I] = raug*(-YftR[I])
                nz += 1
                # (8,1)
                values[nz,I] = raug*(-YftI[I])
                nz += 1

                # 2nd columns
                # (2,2)
                values[nz,I] = param[8,I] + raug
                nz += 1
                # (5,2)
                values[nz,I] = raug*(YffI[I])
                nz += 1
                # (7,2)
                values[nz,I] = raug*(YftI[I])
                nz += 1
                ...
                # 3rd column
                # (3,3)
                values[nz,I] = param[9,I] + raug
                nz += 1
                # (6,3)
                values[nz,I] = raug*(-YttR[I])
                nz += 1
                # (7,3)
                values[nz,I] = raug*(-YtfR[I])
                nz += 1
                # (8,3)
                values[nz,I] = raug*(YtfI[I])
                nz += 1

                # 4th column
                # (4,4)
                values[nz,I] = param[10,I] + raug
                nz += 1
                # (6,4)
                values[nz,I] = raug*(YttI[I])
                nz += 1
                # (7,4)
                values[nz,I] = raug*(YtfI[I])
                nz += 1
                ...
                # 5th column
                # (5,5)
                values[nz,I] = param[11,I] + raug*(YffR[I]^2) + raug*(YffI[I]^2) + raug*(x[6,I]^2)
                nz += 1
                # (6,5)
                values[nz,I] = -(alrect + raug*c5) + raug*(x[5,I]*x[6,I])
                nz += 1
                # (7,5)
                values[nz,I] = raug*(YffR[I]*YftR[I]) + raug*(YffI[I]*YftI[I]) + raug*((-x[6,I])*(2*x[7,I]))
                nz += 1
                # (8,5)
                values[nz,I] = raug*(YffR[I]*YftI[I]) + raug*(YffI[I]*(-YftR[I])) + raug*((-x[6,I])*(2*x[8,I]))
                nz += 1

                # 6th column
                # (6,6)
                values[nz,I] = param[12,I] + raug*(YttR[I]^2) + raug*(YttI[I]^2) + raug*(x[5,I]^2)
                nz += 1
                # (7,6)
                values[nz,I] = raug*(YttR[I]*YtfR[I]) + raug*(YttI[I]*YtfI[I]) + raug*((-x[5,I])*(2*x[7,I]))
                nz += 1
                # (8,6)
                values[nz,I] = raug*((-YttR[I])*YtfI[I]) + raug*(YttI[I]*YtfR[I]) + raug*((-x[5,I])*(2*x[8,I]))
                nz += 1

                # 7th column
                # (7,7)
                values[nz,I] = (alrect + raug*c5)*2 + raug*(YftR[I]^2) + raug*(YftI[I]^2) +
                    raug*(YtfR[I]^2) + raug*(YtfI[I]^2) + raug*((2*x[7,I])*(2*x[7,I]))
                nz += 1
                # (8,7)
                values[nz,I] = raug*(YftR[I]*YftI[I]) + raug*(YftI[I]*(-YftR[I])) + raug*((-YtfR[I])*YtfI[I]) +
                    raug*(YtfI[I]*YtfR[I]) + raug*((2*x[7,I])*(2*x[8,I]))
                nz += 1

                # 8th column
                # (8,8)
                values[nz,I] = (alrect + raug*c5)*2 + raug*(YftI[I]^2) + raug*(YftR[I]^2) +
                    raug*(YtfI[I]^2) + raug*(YtfR[I]^2) + raug*((2*x[8,I])*(2*x[8,I]))
            end
        end
    end
