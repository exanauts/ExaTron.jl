...
#include "gputest_utilities.hpp"

#include "admm_auglag.hpp"
#include "admm_generator.hpp"
#include "admm_bus.hpp"
...
void copy_data(int size, double *dest, double *src)
{
...
void update_multiplier(int size, double *u_curr, double *v_curr,
...
void primal_residual(int size, double *rp, double *u_curr, double *v_curr)
{
    for (int i = 0; i < size; i++) {
...
void dual_residual(int size, double *rd, double *v_prev, double *v_curr, double *rho)
{
    for (int i = 0; i < size; i++) {
...
void read_file(const char *filename, int n, double *data)
{
    ...
    size_t ignore = fread(data, sizeof(double), n, f);
    fclose(f);
}
...
int main(int argc, char **argv)
{
    ...
    while (it < iterlim) {
        ...
        avg_auglag_it = auglag(nw->active_nbranch, n, it, max_auglag, pij_start, qij_start, pji_start, qji_start,
        ...
        printf("%10d\t%.6e\t%.6e\t%.2f\n", it, norm(nvars, rp), norm(nvars, rd), avg_auglag_it);
    }
